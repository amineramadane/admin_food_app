...
class SettingsController extends Controller
{
    ...
    public function whatsapp(){
        $name = env('key_WA_SERVER');
        $data = [];
        try{
            $find = Http::get(env('URL_WA_SERVER').'/session/find/'.$name);
            $cek = json_decode($find->getBody());
            if(isset($cek->success) && $cek->success != false){
                setting(['whatsapp.last_qr' => asset('img/success.png')])->save();
                setting(['whatsapp.status' => 2])->save();
            }
            else{
                setting(['whatsapp.status' => 1])->save();
                $response = Http::post(env('URL_WA_SERVER').'/session/add', 
                    [
                        'id' => $name,
                        'isLegacy' => "false", 
                        "host" => env('APP_URL'), 
                        "keyhost" => env('BOT_ACCESS_KEY_ID')
                    ]
                );
                $res = json_decode($response->getBody());
                if(isset($res->data->qr)) setting(['whatsapp.last_qr' => $res->data->qr])->save();
            }
        }catch(\Exception $e){
            setting(['whatsapp.last_qr' => asset('img/error.png')])->save();
            setting(['whatsapp.status' => 1])->save();
        }
        $data['script_js'] = 'setTimeout(function(){window.location.reload(1);}, 5000);';
        return view('app.settings.whatsapp', ['data' => $data]);
    }
    ...
    public function updateWhatsappSettings(){
        $name = env('key_WA_SERVER');
        $log_out = Http::delete(env('URL_WA_SERVER').'/session/delete/'.$name);
        $cek = json_decode($log_out->getBody());
        if(isset($cek->success) && $cek->success != false){
            setting(['whatsapp.last_qr' => ''])->save();
            setting(['whatsapp.status' => 1])->save();
            Artisan::call('route:clear');
            return redirect('settings/whatsapp')->with('success',__('Log out'));
        }
        Artisan::call('route:clear');
        return redirect('settings/whatsapp')->with('error',__('error'));
    }

    public function sendTestMsgWtsp ( Request $request ) {
        $data = $request->all();

        $validator = Validator::make($data, [
            'send_to_num_wtsp' => ['required','numeric'],
            'test_msg_wtsp' => ['required'],
        ]);
        $validator->validate();

        $name = env('key_WA_SERVER');
        if(!empty(env('URL_WA_SERVER'))) $url = env('URL_WA_SERVER')."/chat/send?id=$name";
        else {
            Artisan::call('route:clear');
            return redirect('settings/bot')->with('error',__('url bot API not found'));
        }
        $res = Http::post($url,
                [
                    'receiver' => $data['send_to_num_wtsp'],
                    'message' => $data['test_msg_wtsp']
                ]
        );
        $cek = json_decode($res->getBody());
        if(isset($cek->success) && $cek->success != false){
            Artisan::call('route:clear');
            return redirect('settings/bot')->with('success',__('success'));
        }
        Artisan::call('route:clear');
        return redirect('settings/bot')->with('error',__('error'));
    }

    public function bot () {
        return view('app.settings.bot');
    }
    public function updateBotSettings (Request $request) {
        ...
        $validator->validate();
        try {
            setting(['bot.message' => $data['message']])->save();
            return redirect('settings/bot')->with('success',__('Message was save successfully!'));
        } catch (\Exception $e) {
            Session::flash('test_email_error_message', $e->getMessage());
            return redirect('settings/bot')->with('error',__('An error occurred while saving message!'));
        }
    }
}
